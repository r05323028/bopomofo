import { roomCleanupIntervalMs, roomTtlMs } from "./constants";
import { getSocketServer } from "./socketServer";
import type {
  GameRoom,
  GuessOutcome,
  HostRoomState,
  Player,
  PlayerAnswer,
  PlayerRoomState,
  PublicRoomState,
} from "./types";
import {
  createId,
  isAnswerComplete,
  normalizeWord,
  sanitizeCell,
} from "./utils";

type CreateRoomInput = {
  topic: string;
  wordCount: number;
};

type JoinRoomInput = {
  roomId: string;
  displayName: string;
};

type GuessAnswerInput = {
  roomId: string;
  guesserId: string;
  targetId: string;
  word: string;
};

class GameRoomManager {
  private readonly rooms = new Map<string, GameRoom>();

  constructor() {
    setInterval(() => {
      this.cleanupExpiredRooms();
    }, roomCleanupIntervalMs).unref();
  }

  createRoom(input: CreateRoomInput): { roomId: string; hostId: string } {
    const topic = input.topic.trim();

    if (!topic) {
      throw new Error("請輸入主題。");
    }

    if (input.wordCount < 1 || input.wordCount > 4) {
      throw new Error("字數必須介於 1 到 4。");
    }

    const roomId = createId("room");
    const hostId = createId("host");

    const room: GameRoom = {
      id: roomId,
      topic,
      wordCount: input.wordCount,
      phase: "lobby",
      hostId,
      players: [],
      answers: {},
      turnOrder: [],
      activePlayerId: null,
      winnerId: null,
      createdAt: Date.now(),
      reveal: {
        guessedComponents: [],
        playerWordRevealed: {},
      },
    };

    this.rooms.set(roomId, room);
    return { roomId, hostId };
  }

  getRoom(roomId: string): GameRoom | null {
    return this.rooms.get(roomId) ?? null;
  }

  getHostRoomState(roomId: string): HostRoomState | null {
    const room = this.getRoom(roomId);
    if (!room) {
      return null;
    }

    return {
      id: room.id,
      topic: room.topic,
      wordCount: room.wordCount,
      phase: room.phase,
      hostId: room.hostId,
      players: room.players,
      answers: room.answers,
      turnOrder: room.turnOrder,
      activePlayerId: room.activePlayerId,
      winnerId: room.winnerId,
      reveal: room.reveal,
    };
  }

  getPlayerRoomState(roomId: string): PlayerRoomState | null {
    const room = this.getRoom(roomId);
    if (!room) {
      return null;
    }

    return {
      id: room.id,
      topic: room.topic,
      wordCount: room.wordCount,
      phase: room.phase,
      players: room.players,
      turnOrder: room.turnOrder,
      activePlayerId: room.activePlayerId,
      winnerId: room.winnerId,
      reveal: {
        guessedComponents: room.reveal.guessedComponents,
      },
    };
  }

  getPublicRoomState(roomId: string, asHost = false): PublicRoomState | null {
    if (asHost) {
      return this.getHostRoomState(roomId);
    }
    return this.getPlayerRoomState(roomId);
  }

  addPlayer(input: JoinRoomInput): Player {
    const room = this.requireRoom(input.roomId);

    if (room.phase !== "lobby") {
      throw new Error("遊戲已經開始。");
    }

    const displayName = input.displayName.trim();
    if (!displayName) {
      throw new Error("請輸入玩家名稱。");
    }

    const nameTaken = room.players.some(
      (player) =>
        player.displayName.localeCompare(displayName, undefined, {
          sensitivity: "base",
        }) === 0,
    );

    if (nameTaken) {
      throw new Error("玩家名稱已被使用。");
    }

    const player: Player = {
      id: createId("player"),
      displayName,
      isEliminated: false,
      hasSubmitted: false,
      joinedAt: Date.now(),
    };

    room.players.push(player);
    room.turnOrder.push(player.id);
    room.reveal.playerWordRevealed[player.id] = false;
    return player;
  }

  submitAnswer(roomId: string, playerId: string, answer: PlayerAnswer): void {
    const room = this.requireRoom(roomId);

    if (room.phase !== "lobby") {
      throw new Error("遊戲開始後無法送出答案。");
    }

    const player = this.requirePlayer(room, playerId);
    const sanitized = answer.map((cell) => sanitizeCell(cell));
    const validation = isAnswerComplete(sanitized, room.wordCount);

    if (!validation.valid) {
      throw new Error("答案尚未填寫完整。");
    }

    room.answers[playerId] = sanitized;
    player.hasSubmitted = true;
  }

  allPlayersSubmitted(roomId: string): boolean {
    const room = this.requireRoom(roomId);
    return (
      room.players.length > 0 &&
      room.players.every((player) => player.hasSubmitted)
    );
  }

  startGame(roomId: string, hostId: string): string {
    const room = this.requireRoom(roomId);

    if (room.hostId !== hostId) {
      throw new Error("只有房主可以開始遊戲。");
    }

    if (room.phase !== "lobby") {
      throw new Error("遊戲已經開始。");
    }

    if (!this.allPlayersSubmitted(roomId)) {
      throw new Error("所有玩家都送出答案後才能開始。");
    }

    room.phase = "in-game";
    room.activePlayerId = this.findNextActivePlayer(room, null);

    if (!room.activePlayerId) {
      throw new Error("沒有可行動的玩家。");
    }

    return room.activePlayerId;
  }

  addGuessedComponent(roomId: string, symbol: string): void {
    const room = this.requireRoom(roomId);
    if (!room.reveal.guessedComponents.includes(symbol)) {
      room.reveal.guessedComponents.push(symbol);
    }
  }

  evaluateAnswerGuess(input: GuessAnswerInput): {
    outcome: GuessOutcome;
    eliminatedPlayerId: string;
  } {
    const room = this.requireRoom(input.roomId);

    const guesser = this.requirePlayer(room, input.guesserId);
    const target = this.requirePlayer(room, input.targetId);

    if (guesser.id === target.id) {
      throw new Error("不能選擇自己作為目標。");
    }

    if (target.isEliminated) {
      throw new Error("目標玩家已被淘汰。");
    }

    const targetAnswer = room.answers[target.id];
    if (!targetAnswer) {
      throw new Error("目標玩家尚未有答案資料。");
    }

    const guessedWord = input.word.trim();
    if (!guessedWord) {
      throw new Error("請輸入要猜測的答案。");
    }

    const normalizedTarget = normalizeWord(targetAnswer);
    const isCorrect =
      guessedWord.localeCompare(normalizedTarget, undefined, {
        sensitivity: "base",
      }) === 0;

    if (isCorrect) {
      this.eliminatePlayer(room.id, target.id);
      return {
        outcome: "correct",
        eliminatedPlayerId: target.id,
      };
    }

    this.eliminatePlayer(room.id, guesser.id);
    return {
      outcome: "wrong",
      eliminatedPlayerId: guesser.id,
    };
  }

  eliminatePlayer(roomId: string, playerId: string): void {
    const room = this.requireRoom(roomId);
    const player = this.requirePlayer(room, playerId);
    player.isEliminated = true;
    room.reveal.playerWordRevealed[player.id] = true;
  }

  advanceTurn(roomId: string): string | null {
    const room = this.requireRoom(roomId);
    room.activePlayerId = this.findNextActivePlayer(room, room.activePlayerId);
    return room.activePlayerId;
  }

  hasWinner(roomId: string): boolean {
    const room = this.requireRoom(roomId);
    return room.players.filter((player) => !player.isEliminated).length <= 1;
  }

  endGame(roomId: string): string | null {
    const room = this.requireRoom(roomId);
    room.phase = "game-over";
    const alive = room.players.find((player) => !player.isEliminated) ?? null;
    room.winnerId = alive?.id ?? null;
    room.activePlayerId = null;
    for (const player of room.players) {
      room.reveal.playerWordRevealed[player.id] = true;
    }
    return room.winnerId;
  }

  cleanupExpiredRooms(now = Date.now()): void {
    for (const [roomId, room] of this.rooms.entries()) {
      if (now - room.createdAt > roomTtlMs) {
        getSocketServer()?.in(roomId).disconnectSockets(true);
        this.rooms.delete(roomId);
      }
    }
  }

  private requireRoom(roomId: string): GameRoom {
    const room = this.rooms.get(roomId);
    if (!room) {
      throw new Error("找不到房間。");
    }
    return room;
  }

  private requirePlayer(room: GameRoom, playerId: string): Player {
    const player = room.players.find((entry) => entry.id === playerId);
    if (!player) {
      throw new Error("找不到玩家。");
    }
    return player;
  }

  private findNextActivePlayer(
    room: GameRoom,
    fromPlayerId: string | null,
  ): string | null {
    const living = room.turnOrder.filter((playerId) => {
      const player = room.players.find((entry) => entry.id === playerId);
      return Boolean(player && !player.isEliminated);
    });

    if (living.length === 0) {
      return null;
    }

    if (!fromPlayerId) {
      return living[0] ?? null;
    }

    const index = living.indexOf(fromPlayerId);
    if (index === -1) {
      return living[0] ?? null;
    }

    return living[(index + 1) % living.length] ?? null;
  }
}

export const gameRoomManager = new GameRoomManager();
